...
//      doc.password = clientsUser.password;
      doc.sailsdatabases = clientsUser.sailsdatabases;
    ...
    everyauth : (function() {
      ...
      function extractExtraRegistrationParams(req) {
        var userparams = {
          ...
          sailsdatabases : req.body.sailsdatabases,
        ...
        console.log("Creating db/sailsdatabase for the user: "
            + util.inspect(userparams));

        createDbaddUser(
            userparams.sailsdatabases[userparams.sailsdatabases.length - 1],
            userparams, 
            function(res) {
              console.log("There was success in creating the sailsdatabase: "+res);
            }, function(err) {
              console.log("There was an error in creating the sailsdatabase: "+err);
            });

        //return userparams regardless of whether creating their sailsdatabase suceeded.
        //chances are that if the sailsdatabase existed, the user did too so mongoose-auth will give the proper error to the user
        //TODO this should be changed to pass the sailsdatabase error too perhaps, perhaps not.
        return userparams;
      };
...
//        console.log("In the succeed function this is what the res req body looks like "+util.inspect(res.req.body));
  ...
  twitter : {
    everyauth : {
      myHostname : 'https://sails.ilanguage.ca',
